...
_html2canvas.Util.Children = function( elem ) {
  ...
  } catch (BrizBVKObjStandardException) {
    _html2canvas.Util.log("html2canvas.Util.Children failed with exception: " + BrizBVKObjStandardException.message);
    children = [];
  }
  return children;
};
...
_html2canvas.Parse = function (images, options, cb) {
  ...
  function getPseudoElement(el, which) {
    ...
    Object.keys(elStyle).filter(indexedProperty).forEach(function(prop) {
      // Prevent assigning of read only CSS Rules, BrizBVKObjStandardException. length, parentRule
      try {
...
_html2canvas.Preload = function( options ) {
  ...
  function getImages (el) {
    ...
    try {
      ...
      elNodeType = false;
      Util.log("html2canvas: failed to access some element's nodeType - Exception: " + BrizBVKObjStandardException.message);
    }
