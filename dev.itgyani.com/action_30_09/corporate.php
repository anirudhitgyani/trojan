<?php include('../connection.php'); 
	...
	if($action == 'insert') {
		...
		$password = $_REQUEST['password'];
               // $corporate_logo = $_FILES['corporate_logo']['name'];
		$created_date = date('Y-m-d H:i:s');
		
		//$NewDBO = date('Y-m-d', strtotime($dateofbirth));
		
		$sql_email = mysql_query("select * from  `profile` where ProfileEmail = '".$email."' ");
		...
		if($total_email=='0') {
			...
			/*$sql = mysql_query("insert into  registered_users set `user_name` = '".$user_name."' , `first_name` = '".$first_name."' , `last_name` = '".$last_name."' , `company_name` = '".$company_name."' , `mobile` = '".$mobile."' , `dateofbirth` = '".$dateofbirth."' , `gender` = '".$gender."' , `email` = '".$email."' , `password` = '".md5($password)."' , `created_date` = '".$created_date."' , `status` = '0' ") or die(mysql_error());*/
                        ...
                        $sql = mysql_query("insert into  `profile` set GroupID = '3' , `ProfileFirstName` = '".$first_name."' , `ProfileLastName` = '".$last_name."' , `ProfileMobile` = '".$mobile."' , `ProfileDBO` = '".$dateofbirth."' ,  `ProfileGender` = '".$gender."' , `ProfileEmail` = '".$email."' , `ProfilePassword` = '".md5($password)."' , ProfileCompanyName = '".$company_name."' , IsFaceBook = '0',IsFaceGmail = '0', IsPaid = '0', IsActive = '1', IsActived='1',IsDeleted='0', AddUserID='0', AddUserTime = '".$created_date."' ") or die(mysql_error());
			$Lastid = mysql_insert_id();
			$_SESSION['Success'] = "Your Register Successfully Done Please check your email";
                        
//                        if($corporate_logo != ''){
//		
//                            $Image = "corporate_logo_".$Lastid.strrchr(basename($corporate_logo),'.');
//                            copy($_FILES["corporate_logo"]["tmp_name"],"../images/corporatelogos/".$Image);
//
//                            $sql = mysql_query("update `profile` set ProfilePicture = '".$Image."' where ProfileID = '".$Lastid."' ");
//                            echo "update `profile` set ProfilePicture = '".$Image."' where ProfileID = '".$Lastid."' ";
//                        }
//			exit();
...
Looking forward to serving you… </p>
	...
	if($action=='login') {
		...
		$sql = mysql_query("select * from `profile` where ProfileEmail = '".$user_name."' AND ProfilePassword = '".md5($password)."' AND GroupID = '3'  ");
	...
	if($action=='forgot_password') {
		...
		if($totalrow_email!='0') {
			...
			$active_link = $RootPath."/pages/corporate/change_password.php?ProfileID=".$row_femail['ProfileID'];
			$message = "
			<html>
			<head>
			...
			<body>
			<p>This email contains to HR Forgot Password Link</p>
	...
	if($action=="edit_profile") {
		...
		if($HrImage != ''){
			...
			$Image = "corporate_logo__".$_SESSION['ProfileID'].strrchr(basename($HrImage),'.');
			copy($_FILES["HrImage"]["tmp_name"],"../images/corporatelogos/".$Image);
